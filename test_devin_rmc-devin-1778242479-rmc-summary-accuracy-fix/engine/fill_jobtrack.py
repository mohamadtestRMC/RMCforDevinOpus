...
def _apply_dominance_filter(mrr_qty: dict, threshold_pct: float = 0.10) -> dict:
    """Filter mrr_qty to keep only dominant MRRs (>= threshold_pct of total).

    Used to drop minor MRRs whose qty share is too small to materially
    affect the qty-weighted rate. Mirrors the manual ground-truth pattern
    of selecting one or two dominant MRRs and ignoring small contributors.

    If no MRR meets the threshold, returns the single MRR with the highest qty.
    """
    if not mrr_qty:
        return mrr_qty
    total = sum(float(q) for q in mrr_qty.values())
    if total <= 0:
        return mrr_qty
    threshold = total * threshold_pct
    dominant = {m: q for m, q in mrr_qty.items() if float(q) >= threshold}
    if not dominant:
        top_mrr = max(mrr_qty, key=lambda k: float(mrr_qty[k]))
        return {top_mrr: mrr_qty[top_mrr]}
    return dominant


def _pick_mrrs_by_total_qty(mrr_qty: dict, target_qty: float, tolerance: float = 1.0):
    """Pick the smallest MRR subset whose total issued qty covers ``target_qty``.

    Used for the FALLBACK (full WO lookup) path where mrr_qty came from
    ``lookup_mrr_with_qty`` and includes every MRR for the WO/material.
    Strategy mirrors the manual ground-truth pattern:
      1. If a single MRR's issue qty is within ``tolerance`` of ``target_qty``,
         return that MRR (exact / near-exact match).
      2. Otherwise sort MRRs by issue qty descending and accumulate them
         (largest first) until cumulative qty >= target_qty. Return that
         minimum cover. This produces:
           - A single dominant MRR when one roll alone covers the target
             (e.g. fallback row using only part of a 376kg roll for 128kg).
           - Multiple MRRs when several rolls were stitched together to
             reach the production qty (e.g. 180+2621+253 to cover 2991).
      3. If even the full set doesn't cover ``target_qty``, return all of them.
    """
    if not mrr_qty:
        return []
    if target_qty <= 0:
        return sorted(int(k) for k in mrr_qty.keys())

    # 1. Exact / near-exact single-MRR match
    best_single_diff = None
    best_single_mrr = None
    for mrr, qty in mrr_qty.items():
        diff = abs(float(qty) - target_qty)
        if best_single_diff is None or diff < best_single_diff:
            best_single_diff = diff
            best_single_mrr = int(mrr)
    if best_single_diff is not None and best_single_diff <= tolerance:
        return [best_single_mrr]

    # 2. Greedy: take largest-qty MRRs first until target is covered
    by_qty_desc = sorted(
        ((float(q), int(m)) for m, q in mrr_qty.items()),
        key=lambda x: (-x[0], x[1]),
    )
    selected = []
    cumul = 0.0
    for qty, mrr in by_qty_desc:
        selected.append(mrr)
        cumul += qty
        if cumul >= target_qty:
            break
...
def _pick_mrrs_formula_matched(mrr_qty: dict, target_qty: float,
                                share_threshold: float = 0.10) -> list:
    """Pick MRRs from a formula-matched mrr_qty dict.

    Used when ``mrr_qty`` came from ``match_formula_qtys_to_store`` — i.e.
    the keys correspond to specific formula components, not to all WO MRRs.
    Strategy mirrors the manual ground-truth pattern:
      1. If the largest single MRR's qty alone covers ``target_qty``, the
         smaller MRRs are likely small leftovers stitched into the formula.
         Drop any MRR whose share of the formula sum is below
         ``share_threshold`` so the qty-weighted rate doesn't get pulled
         by a tiny contributor (e.g. Row 40 in Feb 2026: 93 of 1050.2 = 8.85%).
      2. If no single MRR covers the target, the production row genuinely
         drew from multiple rolls — keep all formula-matched MRRs (e.g.
         Row 45 with 180+2621+253 needed to cover 2991).
    """
    if not mrr_qty:
        return []
    if target_qty <= 0:
    ...
    qtys = {int(m): float(q) for m, q in mrr_qty.items()}
    formula_sum = sum(qtys.values())
    max_qty = max(qtys.values()) if qtys else 0.0

    if formula_sum > 0 and max_qty >= target_qty:
        # Single MRR covers target → drop minor contributors below threshold
        threshold = formula_sum * share_threshold
        kept = {m: q for m, q in qtys.items() if q >= threshold}
        if kept:
            return sorted(kept.keys())
    # Multiple MRRs needed (or threshold dropped everything) → keep all
    return sorted(qtys.keys())
...
def fill_jobtrack(jt_file, stores_file, pr_file, progress_callback=None,
    ...
    for row_idx in range(DATA_START_ROW, max_row + 1):
        ...
        if process_bucket in ('PRINTING', 'REWINDING'):
            ...
            # If formula matching AND balance tracing both produced no MRRs,
            # the formula qty (e.g. =260) didn't align with any single Stores entry.
            # In that case the row's actual material was issued under one of the WO's
            # other MRRs — allow the standard WO lookup to find them.
            if not mrr_qty and skip_fallback:
                skip_fallback = False
            ...
            # When the standard WO lookup returns multiple MRRs, drop minor
            # contributors (< 10% of total qty) so the rate / display reflect
            # the dominant material source rather than averaging across rolls
            # that were never actually used for this production row.
            # Formula-matched MRRs are NEVER filtered — they reflect actual usage.
            if mrr_qty and fallback_used:
                mrr_qty = _apply_dominance_filter(mrr_qty)
            ...
            if mrr_qty:
                # Pick the MRR subset BEFORE rate calculation so the
                # qty-weighted rate only averages over the rolls actually
                # used for this production row. The picking strategy depends
                # on where mrr_qty came from:
                #   - formula_matched=True: keys are precise formula
                #     components → use formula-aware picking (drop minor
                #     contributors only if a single MRR covers the target).
                #   - formula_matched=False: keys came from the full WO
                #     fallback lookup → use greedy minimum-cover picking.
                if formula_matched:
                    mrr_list = _pick_mrrs_formula_matched(mrr_qty, total_input)
                else:
                    mrr_list = _pick_mrrs_by_total_qty(mrr_qty, total_input, tolerance=1.0)
                if mrr_list:
                    mrr_qty = {m: q for m, q in mrr_qty.items() if int(m) in mrr_list}
                ...
                # Re-pick from the (possibly PR-filtered) mrr_qty so that the
                # display only references MRRs that actually contributed to
        ...
        # ========== LAM ROWS: Fresh Materials + Chemicals ==========
            ...
            if fresh1_name and total_fresh1 > 0:
                ...
                # If formula matching AND balance tracing both produced no MRRs,
                # allow the standard WO lookup to find the actual issued material.
                if not mrr_qty and skip_fallback:
                    skip_fallback = False
                ...
                # Drop minor MRRs (< 10% of total) only for fallback lookups,
                # never for formula-matched MRRs (which reflect actual usage).
                if mrr_qty and fallback_used:
                    mrr_qty = _apply_dominance_filter(mrr_qty)
                ...
                if mrr_qty:
                    # Pick the MRR subset BEFORE rate calculation. Strategy
                    # depends on whether mrr_qty came from formula matching
                    # (precise components) or fallback (full WO lookup).
                    if formula_matched:
                        mrr_list = _pick_mrrs_formula_matched(mrr_qty, total_fresh1)
                    else:
                    ...
                    # Re-pick from PR-filtered mrr_qty
                    if mrr_qty:
                        mrr_list = _pick_mrrs_by_total_qty(mrr_qty, total_fresh1, tolerance=1.0)
            ...
            if fresh2_name and total_fresh2 > 0:
                ...
                if mrr_qty:
                    ...
                    # (precise components) or fallback (full WO lookup).
                    if formula_matched:
                        mrr_list = _pick_mrrs_formula_matched(mrr_qty, total_fresh2)
                    else:
